...
final class MeetingRecorder: NSObject {
    ...
    func start(to url: URL, maxDuration: TimeInterval = -1) async throws {
        ...
        // NOTE: SCStream uses the macOS system default input device for its
        // microphone channel and exposes no per-stream device API. WisprAlt
        // intentionally does NOT override the system default. The user-selected
        // mic in the popover applies to dictation only; meeting recording
        // honours whatever the system default is. If the user wants a specific
        // mic for meetings, they set it in System Settings → Sound.
