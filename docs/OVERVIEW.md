...
| `server/src/wispralt_server/routes/dictate.py` | [API.md](API.md) — `/transcribe/dictate` endpoint (incl. `X-Smart-Format` and `X-WisprAlt-Client-Version` header gating); fire-and-forget background INSERT into the `dictations` table after the response is built (skips break-glass admin + empty text). Streaming-dictation Phase 1 added an additive read of the optional `X-Client-Dedup-Id` header (UUID v4 form); when present it's forwarded to `JobStore.insert_dictation(..., client_dedup_id=...)` so the row participates in the `idx_dictations_client_dedup` partial-unique index (ON CONFLICT → no-op). When absent, behavior is bit-identical to pre-streaming. |
| `server/src/wispralt_server/routes/dictate_stream.py` | [API.md](API.md), [ARCHITECTURE.md](ARCHITECTURE.md) — opt-in streaming dictation endpoints (Phase 5b plan). `POST /transcribe/dictate/stream/{session_id}/chunk/{index}` ingests one mid-utterance silence-cut WAV chunk (multipart, 202 with `{received_index, queue_depth}`); `POST /transcribe/dictate/stream/{session_id}/finalize` accepts the tail WAV plus `client_dedup_id` + `speech_started_at` and returns the joined text within a 15 s budget. Mounted between `dictate.router` and `dev_faults.router` so existing dictation paths are unaffected when the feature flag is off. |
| `server/src/wispralt_server/dictate/streaming_session.py` | [ARCHITECTURE.md](ARCHITECTURE.md) — `StreamingSession` dataclass + `StreamingSessionStore` (in-process; 2 max active; one-active-per-`api_key_id`; 900 s sweeper TTL; 15 s finalize budget). Hosts the per-session `asyncio.Lock` that serializes status / `partial_texts` / `pending_tasks` / `last_seen` / `cumulative_audio_ms` mutations, the cumulative-audio 270 s mid-chunk ceiling, and the gap-detection bookkeeping consumed by finalize. Re-uses the single Parakeet executor (no second model instance — see ARCHITECTURE → Streaming dictation). |
...
| `server/src/wispralt_server/routes/me.py` | [API.md](API.md), [ARCHITECTURE.md](ARCHITECTURE.md) — JSON `GET /me` + `PATCH /me` for self-service display name; Phase 2 extends with `GET/POST /me/login` (employee token-paste form) and `GET /me/insights` (per-employee weekly digest + time-range stats, HTMX partial swap on `HX-Request: true`). Plan A adds the 5 `/me/history*` routes: list with per-leg cursor pagination + range/kind/search filters, expand/collapse via `?compact=1`, soft-delete + restore (CSRF-guarded), and per-row `.txt` / `.json` download. See [ARCHITECTURE.md → Personal history](ARCHITECTURE.md#personal-history-mehistory). v0.5.0 adds `GET /me/dictations/last` (JSON, returns the caller's most recent non-deleted dictation; 404 on empty; 403 for break-glass admin) backing the menubar "Copy last dictation" button, and `POST /me/login` now honors a `?next=` URL param (open-redirect-guarded to relative paths under `/me/*` only) so the "Open My Dictations" menubar button can deep-link to `/me/history` after auth. |
...
| `client/WisprAlt/Capture/DictationRecorder.swift` | [ARCHITECTURE.md](ARCHITECTURE.md) — AVAudioEngine dictation. Streaming-dictation Phase 2 adds an additive opt-in branch that feeds tap buffers into `EnergyVAD`; on a silence cut the cut WAV is handed to `DictationStreamSession` for mid-utterance upload. The non-streaming code path (full WAV at FN release) is unchanged. |
| `client/WisprAlt/Capture/EnergyVAD.swift` | [ARCHITECTURE.md](ARCHITECTURE.md) — energy-based voice-activity detector (RMS hysteresis + rolling noise floor + 30 s hard chunk cap + minimum chunk gating). Pure-Swift struct, no Accelerate / no Core ML; consumed by `DictationRecorder` only when `Settings.streamingDictation` is true. |
| `client/WisprAlt/Server/DictationStreamSession.swift` | [ARCHITECTURE.md](ARCHITECTURE.md) — Swift actor that owns one streaming session's UUID, chunk index, dedup id, and `URLSession`. Posts cut chunks to `/transcribe/dictate/stream/{sid}/chunk/{i}` as they're produced, then posts the tail WAV + smart-format flag + `client_dedup_id` + `speech_started_at` to `/finalize` on FN release. Falls back silently to the existing 3-attempt `DictationAPI.transcribe` ladder on any chunk error so the user-facing latency story degrades gracefully. |
...
| `client/WisprAlt/Server/DictationAPI.swift` | [API.md](API.md), [FALLBACK.md](FALLBACK.md) — dictation client + origin→retry→OpenRouter direct fallback. Plan A: `callOpenRouter` enqueues every successful cloud-fallback transcript into `DictationFallbackQueue.shared` (wrapped in `try?` so a queue write failure never breaks the user-facing dictation path). v0.5.0 added the `LastDictationAPI` namespace wrapping `GET /me/dictations/last` (`fetch()` returns `(text, createdAt)`; 404 → friendly "No dictations yet"; 403 → break-glass-admin toast) used by the "Copy last dictation" menubar button. |
...
| `client/WisprAlt/Storage/Settings.swift` | [SETUP-CLIENT.md](SETUP-CLIENT.md), [ARCHITECTURE.md](ARCHITECTURE.md) — UserDefaults keys. Streaming-dictation Phase 2 added the additive `streamingDictation` bool (default `false`); when true AND the recording clears the 8 s minimum-speech bypass, `DictationRecorder` routes chunks through the streaming session. Setting is purely opt-in; off by default keeps the existing single-shot ladder. |
...
| `client/WisprAlt/Update/SparkleController.swift` | [SETUP-CLIENT.md](SETUP-CLIENT.md) — auto-update via Sparkle 2 (deliberately disabled; kept for reference — see `Update/UpdateChecker.swift` for the active v0.5.0 update mechanism) |
| `client/WisprAlt/Update/UpdateChecker.swift` | [ARCHITECTURE.md](ARCHITECTURE.md) — v0.5.0 in-app update checker (polls `releases/latest` 60 s after launch + every 6 h, sets menubar badge via `MenuBarController.setUpdateBadge`, shells `install.sh` curl one-liner through Terminal.app or clipboard-fallback). NOT Sparkle — reuses `install.sh` as canonical install path. |
