...
| **MenuBarController** | `client/WisprAlt/App/MenuBarController.swift` | State machine: idle → dictating → meeting-recording → uploading → processing → done. Enforces mic mutual exclusion. Renders the meeting-recording state via a custom NSImage composite (NSBezierPath red dot + bold "REC" attributed string) drawn into a single bitmap — a previous image+attributedTitle pair was character-wrapping in cramped menubars (R/E/C stacked vertically). Generates human-readable meeting filenames `EEE MMM d h.mm.ssa-h.mm.ssa.wav` (POSIX locale; periods not colons; seconds for sidecar collision avoidance). Uses `AppDelegate.shared?.micMenuBarController?.updateRecordingTint(...)` in `mode.didSet` to keep the second status item's red tint in sync. |
| **MicMenuBarController** | `client/WisprAlt/App/MicMenuBarController.swift` | Second NSStatusItem dedicated to input-device selection. `statusItem.menu = NSMenu(...)` wires left-click to a native dropdown rebuilt fresh on `menuWillOpen`. Items: header, "System Default (current: ...)", separator, all available `AVCaptureDevice` audio inputs (current selection has checkmark), separator, "Open Sound Settings…" footer. Selecting a device sets `Settings.shared.preferredInputDeviceUID`. While recording, `updateRecordingTint(active: true)` keeps `isTemplate=true` and switches `contentTintColor` to `.systemRed`. Empty-list fallback row when mic permission revoked links to `Privacy_Microphone`. Item is gated on `Settings.shared.showMicStatusItem` (toggle in Advanced settings; restart-to-apply). |
| **MicEnumerator** | `client/WisprAlt/Audio/MicEnumerator.swift` | Stateless static helper bridging AVFoundation device discovery to CoreAudio HAL property reads. `availableInputs()` uses `[.external, .microphone]` (macOS 15+ — `.builtInMicrophone` is deprecated, subsumed by `.microphone`); dedup by uniqueID. `audioDeviceID(forUID:)` translates an AV `uniqueID` to an `AudioDeviceID` via `kAudioHardwarePropertyTranslateUIDToDevice` (using `withUnsafePointer(to: &cfUID)` and `qualifierSize = MemoryLayout<CFString?>.size`). `systemDefaultInputDeviceID()` and `setSystemDefaultInputDevice(_:)` read/write `kAudioHardwarePropertyDefaultInputDevice`. `uid(forAudioDeviceID:)` reads `kAudioDevicePropertyDeviceUID` using the in-tree `Unmanaged<CFString>?.takeRetainedValue()` ownership pattern. |
| **AppDelegate** | `client/WisprAlt/App/AppDelegate.swift` | App lifecycle. Exposes `static weak var shared: AppDelegate?` for cross-controller access. On launch: (1) sets `shared`, (2) **mic-override crash recovery** — reads `pendingMeetingDefaultInputUID` from UserDefaults; if present, restores via `MicEnumerator.setSystemDefaultInputDevice(_:)` and clears the key (handles the case where a prior run crashed mid-meeting and left the system input default permanently overridden), (3) constructs `MenuBarController`, (4) constructs `MicMenuBarController` if `Settings.showMicStatusItem`. |
