...
| Route group | Limit |
|---|---|
| `POST /transcribe/dictate` | 60 requests per 60-second window |
| `POST /transcribe/meeting` | 4 requests per 3600-second window |
...
| `formats` | When `status == "done"`. Always `["json", "srt", "txt", "vtt"]`. |
